					...
					if ( is_user_logged_in() ) {
						if ( $project_status == 'publish' ) {
							if ( ( fre_share_role() || $user_role == FREELANCER ) && $user_ID != $project->post_author ) {
								...
								if ( $has_bid ) {
									//NDA - By: eK
									echo '<a href="#" class="fre-normal-btn primary-bg-color bid-action" data-toggle="modal" data-target="#modal_send_nda" data-bid-id="' . $bidding_id . '">' . __( 'My NDA', ET_DOMAIN ) . '</a>';
								...
								//NDA - By: eK
								echo '<a href="#" class="fre-normal-btn primary-bg-color" data-toggle="modal" data-target="#modal_nda_template" data-project-id="' . $project->ID . '">' . __( 'NDA Template', ET_DOMAIN ) . '</a>&nbsp;&nbsp;';
