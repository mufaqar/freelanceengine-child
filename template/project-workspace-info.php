        ...
        <div class="row">
            ...
            <div class="col-lg-4 col-md-5">
                ...
                <div class="project-detail-action">
					<?php
                    // Diputing Block - By: eK
                    $ek_author_id = get_post_field( 'post_author', $project->ID );
                    $ek_args = array(
                        'post_type'      => 'ae_milestone',
                        'posts_per_page' => -1,
                        // 'post_status'    => 'reopen,resolve,done',
                        'post_status'    => 'open',
					...
					if ( $post->post_status == 'close' ) {
						if ( (int) $project->post_author == $user_ID ) { ?>
                            <?php if ( ae_get_option( 'use_escrow' ) ) { ?>
                                <!-- Disputing Block - By: eK -->
                                <?php if( ! $ek_query->have_posts() ){ ?>
                                    <a title="<?php _e( 'Close', ET_DOMAIN ); ?>" href="#" id="<?php the_ID(); ?>"
                                    class="fre-action-btn btn-close-project"><?php _e( 'Dispute', ET_DOMAIN ); ?></a>&nbsp;
                                <?php } ?>
							<?php } ?>
                            <a title="<?php _e( 'Finish', ET_DOMAIN ); ?>" href="#" id="<?php the_ID(); ?>"
                               class="fre-normal-btn primary-bg-color btn-complete-project"> <?php _e( 'Finish', ET_DOMAIN ); ?></a>
						<?php } else {
							if ( $bid_accepted_author == $user_ID && ae_get_option( 'use_escrow' ) ) { ?>
                                <!-- Disputing Block - By: eK -->
                                <?php if( ! $ek_query->have_posts() ){ ?>
                                    <a title="<?php _e( 'Dispute', ET_DOMAIN ); ?>" href="#" id="<?php the_ID(); ?>"
                                    class="fre-action-btn btn-quit-project"><?php _e( 'Dispute', ET_DOMAIN ); ?></a>
                                <?php } ?>
							<?php }
						}
