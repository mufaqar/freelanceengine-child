...
function fre_enqueue_escrow() {
    if (is_singular(PROJECT)) {
        ...
        // Add custom script for initializing Stripe
        wp_enqueue_script('stripe-integration', TEMPLATEURL . '/assets/js/stripe-integration.js', array('stripe-js'), null, true);
    }
}
add_action('wp_enqueue_scripts', 'fre_enqueue_escrow');

/**
 * Handle order meta for Stripe and PayPal
 * @param $order_id
 * @param $payment_method
 */
function fre_handle_order_meta($order_id, $payment_method) {
    update_post_meta($order_id, '_payment_method', $payment_method);

    if ($payment_method === 'stripe') {
        update_post_meta($order_id, '_stripe_payment_status', 'pending');
    } elseif ($payment_method === 'paypal') {
        update_post_meta($order_id, '_paypal_payment_status', 'pending');
    }
}
add_action('fre_process_order', 'fre_handle_order_meta', 10, 2);
...
function fre_get_accept_bid_info() {
    ...
    // Commission - By: eK
    if ($payer_of_commission == 'project_owner' || $payer_of_commission == 'both') {
...
function fre_transfer_money_info(){
    if (ae_get_option('use_escrow')) {
        ...
        if(ae_get_option('manual_transfer')){
            ...
            // Commission - By: eK
            }else if($payer_of_commission == 'worker' || $payer_of_commission == 'both'){
                $total = $bid_budget;
