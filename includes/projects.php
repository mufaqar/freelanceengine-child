		...
		if ( ! is_wp_error( $result ) ) {
			...
			if ( isset( $request['et_payment_package'] ) ) {
				...
				if(  is_acti_fre_membership() ){
					...
					// check recruite_payemtn
					// $is_recuit_pack = get_recruit_pack($pack_sku); pp_subscription
					// if($is_recuit_pack){
					// 	// check it later here, current it alway next to checkoutform for testing.
					// } else {
	...
	public function validate_data( $data ) {
		...
		if ( isset( $data['et_payment_package'] ) ) {
			...
			if ( !$disable_plan &&  ! $plan ) {
				...
				//$plan = get_recruit_pack($data['et_payment_package']);
